...
import {
  ...
  getSubscription,
  getSubscriptionScheduleForSubscription,
  reconcileStaleStripeBilling,
  getOfferedTrialDays,
} from "../services/stripe.mjs";

export async function loadPlanBillingContext(userId) {
  ...
  const starterMonthlyPrice = Number(pricing?.starter?.price || 0);
  const starterYearlyFull = starterMonthlyPrice * 12;
  const starterYearlyPrice = starterMonthlyPrice * 10;
  const starterYearlySavings = starterYearlyFull - starterYearlyPrice;
  const starterTrialDays = stripeEnabled ? await getOfferedTrialDays(plan?.stripe_customer_id || null) : 0;
  ...
  if (stripeEnabled && plan?.stripe_subscription_id) {
    try {
      ...
      if (String(sub?.status || "") === "trialing") {
        isTrialing = true;
        trialEndTs = Number(sub?.trial_end || 0) || null;
      }
  ...
  const paygSummary = await getCurrentMonthPaygOutstanding(userId).catch(() => ({
    overageUnits: 0, overageCents: 0, chargedUnits: 0, chargedCents: 0, outstandingUnits: 0, outstandingCents: 0,
  }));
  ...
  return {
    plan, usage, pricing, starterYearlyFull, starterYearlyPrice, starterYearlySavings, starterTrialDays,
    totalMessages, usagePercentage, currentPlanDetails, stripePublishableKey, paygEnabled, paygPriceText,
    currentPaidInterval, scheduledTargetInterval, scheduledStartTs, willCancelAtEnd, cancelAtTs,
    isTrialing, trialEndTs, isStarterCurrentMonthly, isStarterCurrentYearly, historyRows,
    STARTER_YEARLY_PRICE_ID, STARTER_MONTHLY_PRICE_ID, paygOutstandingCents, paygOutstandingUnits,
    paygBillingHold, subscriptionBillingBlocked, planMonthlyPriceDollars, paygPercentOfPlan,
    paygTotalFormatted, paygOutstandingFormatted, usageBarClass, paygBarClass, messagesRemaining,
    isStarter, stripeEnabled, paygTotalCents,
  };
}

export function renderPlanBillingPanel(ctx) {
  const {
    plan, usage, pricing, starterYearlyFull, starterYearlyPrice, starterYearlySavings, starterTrialDays,
    usagePercentage, currentPlanDetails, paygEnabled, paygPriceText, currentPaidInterval, scheduledTargetInterval,
    scheduledStartTs, willCancelAtEnd, cancelAtTs, isTrialing, trialEndTs, isStarterCurrentMonthly,
    isStarterCurrentYearly, historyRows, paygOutstandingCents, paygOutstandingUnits, paygBillingHold,
    subscriptionBillingBlocked, planMonthlyPriceDollars, paygPercentOfPlan, paygTotalFormatted,
    paygOutstandingFormatted, usageBarClass, paygBarClass, messagesRemaining, isStarter, stripeEnabled, totalMessages,
  } = ctx;
  return `
  ...
  <div class="plan-overview__top">
    <div class="plan-overview__identity">
      ...
      <p class="plan-overview__subtitle">
        ${isTrialing && trialEndTs
          ? `Free trial until ${new Date(trialEndTs * 1000).toLocaleDateString()}`
          : `${planMonthlyPriceDollars > 0 ? `$${planMonthlyPriceDollars}/month` : "No monthly fee"} · ${plan.monthly_limit.toLocaleString()} messages included`}
      </p>
    </div>
    ...
    ${Object.entries(pricing).map(([planKey, planDetails]) => `
      <div class="plan-option ${plan.plan_name === planKey ? 'current' : ''}">
        ...
        <div class="plan-price">
          ...
          <span class="price-yearly">
            ${planKey === 'starter' ? `
              <span class="plan-price-yearly">
                <span class="plan-price-strike">$${starterYearlyFull}</span>
                <strong>$${starterYearlyPrice}</strong><span class="plan-price-period">/year</span>
                <span class="plan-badge-discount">Save $${starterYearlySavings}</span>
              </span>
            ` : `
          ...
          ` : ``}
          ${planKey === 'starter' && starterTrialDays > 0 && plan.plan_name !== 'starter' ? `
            <p class="plan-trial-note">${starterTrialDays}-day free trial</p>
          ` : ''}
        </div>
        <div class="cta-row">
          ${plan.plan_name !== planKey ? `
            ${stripeEnabled && planKey !== 'free' ? `
              <button class="btn btn-primary btn-full" onclick="subscribeToPlan('${planKey}')">
                <span class="cta-monthly">${starterTrialDays > 0 && planKey === 'starter' ? `Start ${starterTrialDays}-day free trial` : `Subscribe to ${planDetails.name}`}</span>
                <span class="cta-yearly">${starterTrialDays > 0 && planKey === 'starter' ? `Start ${starterTrialDays}-day free trial` : `Subscribe to ${planKey === 'starter' ? 'Starter Yearly' : planDetails.name}`}</span>
              </button>
            ` : `
...
export function renderPlanBillingScripts(ctx) {
  const {
    stripeEnabled, stripePublishableKey, STARTER_YEARLY_PRICE_ID, STARTER_MONTHLY_PRICE_ID,
    currentPaidInterval, scheduledTargetInterval, scheduledStartTs, paygEnabled, paygOutstandingCents, plan,
  } = ctx;
  return `
  ${stripeEnabled ? `const stripe = Stripe('${escapeHtml(stripePublishableKey || "")}');` : ''}
  ...
  // Handle Stripe return query params gracefully (only when returning from checkout)
    ...
    try {
      ...
      if (canceled === 'true') {
        ...
        if (PAYG_OUTSTANDING_CENTS > 0) {
          ...
          try { history.replaceState({}, document.title, location.pathname + location.search + '#billing'); } catch {}
          return;
        }
      }
      ...
      if (title) {
        Modal.alert({ title, message }).then(goBilling);
      } else {
        goBilling();
      }
    } catch {}
